...
%             setparam(obj.bb.tg, 'STFTSpindle4', 'sigma_threshold', 0.2);
        ...
        function armPulse(obj)
            ...
            %  call the tpm function 
            if contains(obj.best_toolbox.app.info.event.current_measure_fullstr,'SpindleEventFree')
                 obj.bb.configure_time_port_marker([0 4 1])
            ...
            pause(4);                                %%% FB temp hardcoded
            obj.bb.arm;
            
            while obj.bb.triggers_remaining ~= 0
                if contains( obj.best_toolbox.app.info.event.current_measure_fullstr,'SpindleEventFree')
                    obj.spindleevent_offset_stim;
                end
                pause(0.1);
            end
...
%                 end
        ...
        function spindleevent_offset_stim(obj)
            disp('Entered succwfully =--------------------------------------------------')
            obj.bb.triggers_remaining = 0;
            spindleevent_offset_stim_exit=0;
            % obj.bb is boss
            % obj.best_toolbox is obj in best_toobox
            if contains( obj.best_toolbox.app.info.event.current_measure_fullstr,'SpindleEventFree') % NEED to check if ths is ported here correctly
                time_port_marker_vector=obj.best_toolbox.inputs.trialMat{obj.best_toolbox.inputs.trial,obj.best_toolbox.inputs.colLabel.tpm};
                obj.bb.configure_time_port_marker(cell2mat(time_port_marker_vector'))
                pause(2.9)
%                 obj.best_toolbox.inputs.stop_event=0;
                while spindleevent_offset_stim_exit==0

                        
                    spindleflag=getsignal(obj.bb.tg,'STFTSpindle4/spindle_present1');drawnow; pause(0.1);
                    spindleflag
                    if spindleflag==0
                         disp('Exited if succwfully =--------------------------------------------------')
                        obj.bb.manualTrigger;
                        spindleevent_offset_stim_exit=1;
                        obj.bb.triggers_remaining = 0;
                        break;
                    end
                    
                    if obj.best_toolbox.inputs.stop_event==1
                        spindleevent_offset_stim_exit=1;
                         obj.bb.triggers_remaining = 0;
                        break;
                    end
