    ...
    methods
        ...
        function trigTrial(obj)
            switch obj.app.par.hardware_settings.(char(obj.inputs.trialMat{obj.inputs.trial,obj.inputs.colLabel.outputDevices}{1,1})).slct_device
                case 1 % pc controlled magven % adding neurofus control has a prototype layer
                case 2 % pc controlled magstim
                case 3 % pc controlled bistim
                case 4 % pc controlled rapid
                ...
                case 11 %bossbox controlled usb triggered neurofus
                    switch obj.inputs.BrainState
                        case 1
                            obj.bossbox.bb.sendPulse(8);
                            trigger(obj.bossbox.EMGScope)
                            obj.neurofus.system_state=1;
                        case 2
                            obj.bossbox.armPulse;
                            obj.bossbox.bb.triggers_remaining
                    ...
                    for i=1:numel(unique_chLab)
                        switch obj.inputs.trialMat{obj.inputs.trial,obj.inputs.colLabel.chType}{1,i}
                            case 'IP'
                                ...
                                disp('Read IP scope successfully')
                            case 'IEEG'
                                ...
                                obj.inputs.rawData.(unique_chLab{1,i}).data(obj.inputs.trial,:)=obj.bossbox.IEEGScopeRead;
                                 disp('Read IEEG scope successfully')
                            ...
                            case 'EMG'
                                ...
                                % NewUseThis obj.inputs.rawData.(unique_chLab{1,i}).data(obj.inputs.trial,:)=obj.best_VisualizationFilter(obj.bossbox.EMGScopeRead(obj.inputs.trialMat{obj.inputs.trial,obj.inputs.colLabel.chID}{1,i}))   %%[obj.bossbox.EMGScope.Data(:,1)]');
                                 disp('Read EMG scope successfully')
                            case 'EEG'
                                ...
                                obj.inputs.rawdata.trialdef=obj.inputs.trial;
                                 disp('Read EEG scope successfully')
                            case 'EEG FieldTrip'
                                if i==1
...
%                                         obj.neurofus.power=obj.inputs.trialMat{obj.inputs.trial,obj.inputs.colLabel.GlobalPower};
%                                         obj.neurofus.arm;

%                                       added by Mihir on 20th July 2025
                                        centr_freq = 10;
                                        num_repetitions = 10;
                                        ...
                                        global_power= 10000; %mW -> 15960 / 30 W/cm^2 intensity
                                        %           global_power=23940; %mW -> 45 W/cm^2 intensity
                                        %           global_power=31915; %mW -> 60 W/cm^2 intensity (seems to be the maximum)

                                        obj.neurofus.burst_length = burst_duration*10e2; %us; 80000 + 10000 due to ramping!
                                        obj.neurofus.period = repetition_interval*10e2; %us PRI
                                        obj.neurofus.timer = pulse_train_duration*10e2; %us PTD
                                        obj.neurofus.global_power = global_power; %mW -> 60 W/cm^2 intensity (seems to be the maximum)
                                        obj.neurofus.focus=30000;
                                        obj.neurofus.arm;
%                                         obj.bossbox.bb.sendPulse(8)
                                    catch
        ...
        function boot_neurofus(obj)
            ...
            delete(instrfindall)
%             b=bossdevice;
            obj.neurofus=neurofus('COM6');
            obj.neurofus.connect;
            pause(2)
            obj.neurofus.ramp_mode = 0;
            obj.neurofus.trigger_mode=4;
            ...
            burst_duration = round(repetition_interval/4); %ms
            pulse_train_duration = num_repetitions * repetition_interval; %ms
            global_power= 15960; %mW -> 15960 / 30 W/cm^2 intensity
%           global_power=23940; %mW -> 45 W/cm^2 intensity
%           global_power=31915; %mW -> 60 W/cm^2 intensity (seems to be the maximum)
            ...
            obj.neurofus.period = repetition_interval*10e2; %us PRI
            obj.neurofus.timer = pulse_train_duration*10e2; %us PTD
            obj.neurofus.global_power = 1000; %mW -> 60 W/cm^2 intensity (seems to be the maximum)
            obj.neurofus.focus=30000;

            obj.neurofus.global_frequency=250000;  %fundamental frequency 545kHz (545000)
            obj.neurofus.system_state=1;
            pause(0.01)
            ...
            %             pause(2);
            %             obj.neurofus.trigger_mode=4;
            %             obj.neurofus.ramp_mode = 0; % tukey ramp (1=linear, 2=tukey, 3=log, 4=exponential)
...
%             obj.bossbox.bb.sendPulse(8)
            ...
            % Copied from NSL1 BEST toolbox 12/02/2025
            % below code hard-codes NeuroFUS to start in ramp enabled
            % trigger mode, so does not use BOSS device to trigger.
            % Instead uses USB.
            %           obj.neurofus=neurofus(obj.app.par.hardware_settings.(char(obj.inputs.output_device)).comport);
            %           obj.neurofus.connect;
            %
