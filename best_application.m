...
%             obj.pi.tmsfmri.volumes_vector_full.Enable='off';
            ...
            if(obj.pi.tmsfmri.block_design.Value==1)
                ...
                trial_vector=obj.pi.tmsfmri.vol_vect(1,:);
                vol_vector= obj.pi.tmsfmri.vol_vect(1,:);
                
                if size(obj.pi.tmsfmri.vol_vect,1)==2
                    intensity_vector=obj.pi.tmsfmri.vol_vect(2,:);
                    trial_vector(2,:)=obj.pi.tmsfmri.vol_vect(2,:)
                else
                    intensity_cond = obj.par.(obj.info.event.current_session).(obj.info.event.current_measure_fullstr).stimulation_intensities;
                    intensity_cond_rep=numel(obj.pi.tmsfmri.vol_vect(1,:));
            ...
            intensitycheck=obj.par.(obj.info.event.current_session).(obj.info.event.current_measure_fullstr).manual_stim_inten
...
%                             end
                ...
                set(obj.pi.tmsfmri.status,'String','Completed!');drawnow
            else
                if(obj.par.(obj.info.event.current_session).(obj.info.event.current_measure_fullstr).manual_stim_inten==1)
                    ...
                    magventureObject.setAmplitude(round(intensity_vector(1)));
                    set(obj.pi.tmsfmri.status,'String','Ready!');drawnow
...
% %                     xlabel('Time (s)');
        ...
        function usb_triggered_neurofus
            delete(instrfindall)
            b=bossdevice;
            n=neurofus('COM6');
            n.connect;
            ...
            n.arm;
            b.sendPulse(4);
            n.global_power= 15960; %mW -> 30 W/cm^2 intensity
